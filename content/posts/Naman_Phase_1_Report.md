...
Week one was all about the database. I decided early that the smart move was to get the foundation rock solid before building anything on top of it. I raised the PRs for the **`organizations`** and **`organization_members`** tables, along with the necessary alterations to the existing `users` and `groups` tables so everything fit together.
...
![DB](/images/naman_gsoc_2026/DB.png)

### Sprint 2 (1–7 Jun): Wrapping the data layer

The `organization_members` PR came together and wrapped up the database layer cleanly. With the schema in place, I put up the **models** PR and lined up the **policy** and **controller** work next. I had a dedicated call scheduled with Pratham Bhaiya to go over the authorization direction before writing it, with the plan being to get the policy PR out before the next Wednesday meet, no delays.

A key design decision landed here: a user should never be able to join the same organization twice or hold two roles in it. I enforced this on both layers, with a model validation *and* a matching unique database index, so duplicates are impossible even under a race.

- **PR:** [Organization & OrganizationMember models + associations](https://github.com/CircuitVerse/CircuitVerse/pull/7451)
![Models](/images/naman_gsoc_2026/models.png)

### Sprint 3 (8–14 Jun): Authorization & RBAC

I had a focused **RBAC call with Pratham Bhaiya** to plan the policy layer, covering how roles and permissions should be structured, before writing a single line. That conversation saved a lot of guesswork. I then raised the **policy PR** with its accompanying specs, along with the **controllers**, and in the Wednesday meet we talked through which edge cases mattered now versus later. The call was clear: establish the foundation first, handle edge cases after, with no over-engineering before the base is solid.
...
![RBAC Matrix](/images/naman_gsoc_2026/RBAC.png)
![Controller](/images/naman_gsoc_2026/Controller_ER_Diagram.png)
### Sprint 4 (15–21 Jun): Into the frontend

With the backend written and in review, I moved ahead and started on the **views**. Most of the UI came together quickly, but the number of edge cases I had to reason through on my own genuinely surprised me. Building an interface where actions are gated by role, states change based on data, and nothing breaks for a standalone versus an organization-owned resource is a lot more subtle than it looks from the outside. But it came together.

I also added the **`location`** field to organizations end to end: the migration, model validation (max 100 chars), and the UI.
...
This sprint the frontend really took shape. I finished the views I had planned, but the first pass was functional rather than polished. I took it to the mentor meet, got solid direction on the design, reworked it, then raised the **Organization Dashboard PR**. It went from "works" to genuinely looking the part, closely matching what my mentor had described.
...
![The organization dashboard](/images/naman_gsoc_2026/dashboard.png)

### Sprint 6 (29 Jun–5 Jul): Splitting PRs & member management

Feedback from my mentors this sprint pushed me to **split my large dashboard PR into small, focused, stacked PRs** (one for the dashboard, one for the settings page, one for member management), which made reviews far faster and sharper. I worked through a detailed round of review comments across them, several of them security related: gating the organization param behind the feature flag on create only (to prevent org-hijacking through the edit form), making the "leave organization" action fail gracefully instead of erroring, and guarding the group authorization against invalid input.
